...
export default function TeamSection() {
  ...
  // Show 4 profiles visible at once on desktop
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 border-b-2 border-[var(--border-main)] pb-6 gap-6">
          ...
          <div className="font-mono text-xs text-[var(--text-muted)] font-bold uppercase">
            ACTIVE PROFILE <span className="text-[#da261c]">{String(currentIndex + 1).padStart(2, '0')}</span> / {String(total).padStart(2, '0')}
          </div>
        </div>

        {/* Carousel Container with Floating Arrows on Left & Right Sides */}
        <div className="relative px-2 sm:px-12">
          ...
          <button
            ...
            className="absolute left-0 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-12 md:h-12 bg-[var(--bg-surface)] hover:bg-[#da261c] hover:text-white text-[var(--text-main)] border-2 border-black shadow-[3px_3px_0px_rgba(0,0,0,0.9)] active:translate-y-0.5 transition-all flex items-center justify-center font-mono font-bold shrink-0"
          ...
          <button
            ...
            className="absolute right-0 top-1/2 -translate-y-1/2 z-20 w-10 h-10 md:w-12 md:h-12 bg-[var(--bg-surface)] hover:bg-[#da261c] hover:text-white text-[var(--text-main)] border-2 border-black shadow-[3px_3px_0px_rgba(0,0,0,0.9)] active:translate-y-0.5 transition-all flex items-center justify-center font-mono font-bold shrink-0"
            aria-label="Next Member"
          >
            <ChevronRight className="w-6 h-6" />
          </button>

          {/* Carousel Track: 4 cards visible at once, sliding 1-by-1 */}
          <div className="overflow-hidden">
            ...
            >
              ...
              {[...TEAM_DATA, ...TEAM_DATA].map((member, idx) => (
                <div
                  key={`${member.id}-${idx}`}
                  className="w-full min-w-[100%] sm:min-w-[48%] lg:min-w-[calc(25%-18px)] bg-[var(--bg-surface)] border-2 border-[var(--border-main)] hover:border-[#da261c] p-5 transition-all duration-200 group flex flex-col justify-between shadow-[4px_4px_0px_rgba(0,0,0,0.15)] shrink-0"
                >
                  <div>
                    {/* Member Avatar & Basic Info */}
                    <div className="flex items-center gap-3 mb-4">
                      <div className="w-16 h-16 border-2 border-black p-0.5 bg-[var(--bg-surface-subtle)] shrink-0 overflow-hidden shadow-[2px_2px_0px_rgba(0,0,0,0.8)]">
                        <img
                          src={member.avatar}
                          alt={member.name}
                      ...
                      <div className="flex flex-col">
                        <span className="font-mono text-[9px] text-[#da261c] font-black tracking-wider uppercase">
                          {member.role}
                        </span>
                        <h3 className="font-headline font-black text-lg text-[var(--text-main)] group-hover:text-[#da261c] transition-colors uppercase leading-tight">
                          {member.name}
                        </h3>
                        <span className="font-mono text-[11px] text-[var(--text-muted)] font-bold">
                          {member.handle}
                        </span>
                        <div className="inline-flex items-center gap-1 mt-1 font-mono text-[10px] text-[#001dc2] font-black">
