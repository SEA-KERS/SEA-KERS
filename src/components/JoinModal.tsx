...
export default function JoinModal({ isOpen, onClose }: JoinModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-(--bg-surface) border-2 border-black max-w-lg w-full p-6 md:p-8 relative shadow-[8px_8px_0px_rgba(0,0,0,0.9)]">
        <button
          onClick={onClose}
        ...
        {!submitted ? (
          <div>
            ...
            <h3 className="font-headline font-black text-2xl text-(--text-main) mb-2 uppercase">
              Join The Collective
            </h3>

            <p className="font-headline text-sm text-(--text-muted) font-normal mb-6">
              We operate as a flat, meritocratic collective. Access private
              repos, join 48-hour sprints, and build hard protocol
              infrastructure.
            </p>
            ...
            >
              <div>
                <label
                  htmlFor="join-handle"
                  className="text-xs text-(--text-muted) uppercase block mb-1 font-bold"
                >
                  NAME / HANDLE:
                </label>
                <input
                  ...
                  className="w-full bg-(--bg-surface-subtle) border-2 border-(--border-main) focus:border-[#da261c] p-3 text-sm text-(--text-main) outline-none font-bold"
                />
              </div>
              ...
              <div>
                ...
                >
                  <option>
                    Systems & Distributed Engineer (Rust / Go / C++)
                  </option>
                  <option>Zero-Knowledge Cryptographer (Circom / Halo2)</option>
                  <option>
                    AI & Neural Security Researcher (PyTorch / LLMs)
                  </option>
          ...
          <div className="text-center py-8">
            ...
            <h3 className="font-headline font-black text-2xl text-(--text-main) uppercase mb-2">
              TRANSMISSION RECEIVED
            </h3>
            <p className="font-mono text-xs text-(--text-muted) font-bold">
              Your proof of work has been logged. Core engineers will initiate
              contact.
            </p>
          </div>
        )}
